...
resource "aws_autoscaling_group" "autoscaling_disabled" {
 count = var.create && !var.create_autoscaling_group ? 1 : 0
...
# Credits: https://github.com/bryantbiggs/eks-desired-size-hack
################################################################################
  ...
  provisioner "local-exec" {
    ...
    command     = <<-EOT
      aws autoscaling update-auto-scaling-group \
        --auto-scaling-group-name ${aws_autoscaling_group.this[0].name} \
        --desired-capacity ${var.desired_size} \
        --region ${var.region}
    EOT
  }

...
resource "aws_eks_cluster" "this" {
  ...
  access_config {
    ...
    #bootstrap_cluster_creator_admin_permissions = var.authentication_mode == "CONFIG_MAP" ? true : false
  ...
  dynamic "encryption_config" {
    ...
    content {
      provider {
        key_arn = var.create_kms_key ? module.kms.key_arn : encryption_config.value.provider.key_arn
      }
  ...
  flattened_access_entries = flatten([
    for entry_key, entry_val in local.merged_access_entries : [
      ...
      merge(
        ...
        } : k => v if !contains(["EC2_LINUX", "EC2_WINDOWS", "FARGATE_LINUX", "HYBRID_LINUX", "HYPERPOD_LINUX"], lookup(entry_val, "type", "STANDARD")) },
      )
    ]
  ])
}
    ...
    Statement = [
      {
        ...
        Effect   = "Allow"
        Resource = var.create_kms_key ? module.kms.key_arn : var.cluster_encryption_config.provider.key_arn
      },
    ]
  })
